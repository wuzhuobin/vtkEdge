...
// Fragment program part with ray cast and 4-dependent-component composite
// method.
...
void trace(void)
{
  ...
  while (inside)
    ...
    while(inside)
      {
      // color is the sampled texture value in the volume at pos
      color=texture3D(dataSetTexture,pos);
      
      // opacity is the sampled texture value in the 1D opacity texture at
      // color.a
      opacity=texture1D(opacityTexture,color.a);
  ...
  gl_FragColor.r = destColor.r;
  gl_FragColor.g = destColor.g;
  gl_FragColor.b = destColor.b;
