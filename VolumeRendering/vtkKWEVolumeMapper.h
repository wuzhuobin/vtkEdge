...
//-----------------------------------------------------------------------------
// .NAME vtkKWEVolumeMapper - Adaptive volume mapper
// .SECTION Description
// vtkKWEVolumeMapper is a volume mapper that will delegate to a specific
...
//          Allow the vtkKWEVolumeMapper to select the best mapper based on
...
//
//  vtkKWEVolumeMapper::TextureRenderMode: 
...
//
//  vtkKWEVolumeMapper::GPURenderMode: 
//          Use the vtkKWEGPUVolumeRayCastMapper, if supported, for both
...
#ifndef __vtkKWEVolumeMapper_h
#define __vtkKWEVolumeMapper_h

#include "vtkVolumeMapper.h"
#include "VTKEdgeConfigure.h" // include configuration header

class vtkFixedPointVolumeRayCastMapper;
class vtkKWEGPUVolumeRayCastMapper;
...
class VTKEdge_VOLUMERENDERING_EXPORT vtkKWEVolumeMapper : public vtkVolumeMapper
{
public:
  static vtkKWEVolumeMapper *New();
  vtkTypeRevisionMacro(vtkKWEVolumeMapper,vtkVolumeMapper);
...
//ETX  
  ...
  // Description:
  // Set the requested render mode to vtkKWEVolumeMapper::DefaultRenderMode.
  ...
  void SetRequestedRenderModeToDefault()
    { this->SetRequestedRenderMode( vtkKWEVolumeMapper::DefaultRenderMode ); }
  
  // Description:
  // Set the requested render mode to vtkKWEVolumeMapper::RayCastAndTextureRenderMode.
  // This is a good option if you want to avoid using advanced OpenGL functionality,
  // but would still like to used 3D texture mapping, if available, for interactive
  // rendering.
  void SetRequestedRenderModeToRayCastAndTexture()
    { this->SetRequestedRenderMode( vtkKWEVolumeMapper::RayCastAndTextureRenderMode ); }
  
  
  // Description:
  // Set the requested render mode to vtkKWEVolumeMapper::RayCastRenderMode.
  ...
  void SetRequestedRenderModeToRayCast()
    { this->SetRequestedRenderMode( vtkKWEVolumeMapper::RayCastRenderMode ); }
...
protected:
  ...
  void  Initialize(vtkRenderWindow *win,
                   vtkVolume *vol);
  ...
  void  ComputeRenderMode(vtkRenderWindow *win,
                          vtkVolume *vol);
  
  // The three potential mappers
  vtkKWEGPUVolumeRayCastMapper      *GPULowResMapper;
  vtkKWEGPUVolumeRayCastMapper      *GPUMapper;
...
private:
  vtkKWEVolumeMapper(const vtkKWEVolumeMapper&);  // Not implemented.
  void operator=(const vtkKWEVolumeMapper&);  // Not implemented.
};
