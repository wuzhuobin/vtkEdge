...
//----------------------------------------------------------------------------
class vtkKWEDensifyPolyDataInternals
{
public:
  ...
  vtkKWEDensifyPolyDataInternals( double    * verts,
                                    vtkIdType   numVerts, 
                                    vtkIdType * vertIds,
                                    vtkIdType  & numPoints,
...
//----------------------------------------------------------------------------
vtkCxxRevisionMacro(vtkKWEDensifyPolyData, "$Revision: 710 $");
vtkStandardNewMacro(vtkKWEDensifyPolyData);

//----------------------------------------------------------------------------
vtkKWEDensifyPolyData::vtkKWEDensifyPolyData()
{
...
//----------------------------------------------------------------------------
int vtkKWEDensifyPolyData::RequestData(
  ...
  vtkInformationVector *outputVector)
{
  ...
  if (!inputPolys || !inputPoints)
    {
    vtkWarningMacro(
      "vtkKWEDensifyPolyData has no points/cells to linearly interpolate.");
    return 0;
    }
  ...
  for (inputPolys->InitTraversal();
    ...
    if (nSubdivisions == 0 || nSubdivisions == VTK_UNSIGNED_INT_MAX)
      ...
      vtkKWEDensifyPolyDataInternals 
...
//----------------------------------------------------------------------------
int vtkKWEDensifyPolyData::FillInputPortInformation(
...
//----------------------------------------------------------------------------
void vtkKWEDensifyPolyData::PrintSelf(ostream& os, vtkIndent indent)
{
