...
extern const char* vtkKWESurfaceLICPainter_fs1;
extern const char* vtkKWESurfaceLICPainter_vs1;
extern const char* vtkKWESurfaceLICPainter_fs2;
...
class vtkKWESurfaceLICPainter::vtkInternals
{
public:
...
vtkStandardNewMacro(vtkKWESurfaceLICPainter);
vtkCxxRevisionMacro(vtkKWESurfaceLICPainter, "$Revision: 710 $");
//----------------------------------------------------------------------------
vtkKWESurfaceLICPainter::vtkKWESurfaceLICPainter()
{
...
//----------------------------------------------------------------------------
void vtkKWESurfaceLICPainter::SetInputArrayToProcess(int fieldAssociation,
  const char* name)
{
...
//----------------------------------------------------------------------------
void vtkKWESurfaceLICPainter::ReleaseGraphicsResources(vtkWindow* win)
{
...
//----------------------------------------------------------------------------
bool vtkKWESurfaceLICPainter::CanRenderLIC(vtkRenderer *vtkNotUsed(renderer),
                                           vtkActor *actor)
{
...
//----------------------------------------------------------------------------
bool vtkKWESurfaceLICPainter::IsSupported(vtkRenderWindow* renWin)
{
  return (vtkKWEDataTransferHelper::IsSupported(renWin) &&
    vtkKWELineIntergralConvolution2D::IsSupported(renWin));
}

//----------------------------------------------------------------------------
void vtkKWESurfaceLICPainter::PrepareForRendering(vtkRenderer* renderer, vtkActor* actor)
{
  this->PrepareOutput();
  ...
  if (!this->Internals->PassTwo)
    ...
    s3->SetSourceCode(vtkKWESurfaceLICPainter_fs2);
...
//----------------------------------------------------------------------------
void vtkKWESurfaceLICPainter::RenderInternal(vtkRenderer *renderer,
                                             vtkActor *actor,
                                             ...
                                             bool forceCompileOnly)
{
  ...
  // vtkKWELineIntergralConvolution2D needs step size in normalized image space, so we
  ...
  vtkKWELineIntergralConvolution2D* licer = vtkKWELineIntergralConvolution2D::New();
  licer->SetSteps(this->NumberOfSteps);
  licer->SetStepSize(stepsize);
  licer->SetNoise(this->Internals->NoiseImage);
  licer->SetVectorField(this->Internals->VelocityImage);
  licer->SetComponents(0, 1);
  ...
  // vtkKWELineIntergralConvolution2D changed the matrices to be orthogonal to the
...
//-----------------------------------------------------------------------------
void vtkKWESurfaceLICPainter::ReportReferences(vtkGarbageCollector *collector)
{
...
//----------------------------------------------------------------------------
vtkDataObject* vtkKWESurfaceLICPainter::GetOutput()
{
  if (this->Enable)
    {
    return this->Output;
...
//----------------------------------------------------------------------------
bool vtkKWESurfaceLICPainter::PrepareOutput()
{
  if (!this->Enable)
    {
...
//----------------------------------------------------------------------------
void vtkKWESurfaceLICPainter::PrintSelf(ostream& os, vtkIndent indent)
{
