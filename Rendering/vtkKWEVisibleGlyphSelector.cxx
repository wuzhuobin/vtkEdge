...
//----------------------------------------------------------------------------
class vtkKWEVisibleGlyphSelectorInternals
{
...
public:
  ...
  vtkKWEVisibleGlyphSelectorInternals()
    {
    memset(Byte,0,15);
    this->PixelCount = 0;
    }
  ...
  //hit records.
  bool operator<(const vtkKWEVisibleGlyphSelectorInternals other) const
    {
    for (int i = 0; i < 15; i++)
      {
...
//////////////////////////////////////////////////////////////////////////////
vtkCxxRevisionMacro(vtkKWEVisibleGlyphSelector, "$Revision: 510 $");
vtkStandardNewMacro(vtkKWEVisibleGlyphSelector);
vtkCxxSetObjectMacro(vtkKWEVisibleGlyphSelector, Renderer, vtkRenderer);

//-----------------------------------------------------------------------------
vtkKWEVisibleGlyphSelector::vtkKWEVisibleGlyphSelector()
{
  this->Renderer = NULL;
...
//----------------------------------------------------------------------------
void vtkKWEVisibleGlyphSelector::SetArea(unsigned int x0, unsigned int y0,
                                     unsigned int x1, unsigned int y1)
{
  if (this->Renderer == NULL)
    {
    vtkErrorMacro("vtkKWEVisibleGlyphSelector must have a vtkRenderer assigned.");
    return;    
    }
...
//----------------------------------------------------------------------------
void vtkKWEVisibleGlyphSelector::GetArea(unsigned int &x0, unsigned int &y0,
                                     unsigned int &x1, unsigned int &y1)
{ 
  x0 = this->X0;
  x1 = this->X1;
...
//----------------------------------------------------------------------------
void vtkKWEVisibleGlyphSelector::SetProcessorId(unsigned int pid)
{
...
//----------------------------------------------------------------------------
void vtkKWEVisibleGlyphSelector::SetRenderPasses(int p, 
                                             int a, 
                                             int h, int m, int l)
{
  this->DoProcessor = p;
...
//-----------------------------------------------------------------------------
void vtkKWEVisibleGlyphSelector::SetSelectMode(int mode)
{
  if (this->Delegate == NULL)
    {
    return;
    }
  this->Delegate->SetSelectMode(mode);
}

//-----------------------------------------------------------------------------
void vtkKWEVisibleGlyphSelector::SetSelectConst(unsigned int constant)
{
  if (this->Delegate == NULL)
    {
    return;
    }
  this->Delegate->SetSelectConst(constant);
}

//----------------------------------------------------------------------------
void vtkKWEVisibleGlyphSelector::Select()
{
  ...
  if(this->Delegate==0)
    {
      this->Delegate=vtkKWEGlyphSelectionRenderMode::New();
      this->Delegate->SetUsed(true);
    }

  this->Renderer->SetDelegate(this->Delegate);
  ...
  for (int i = 0; i < 5; i++)
    ...
    if (i==2 && !this->DoGlyphIdHi)
      {
      continue;
      }
    if (i==3 && !this->DoGlyphIdMid)
      {
      continue;
      }
    if (i==4 && !this->DoGlyphIdLo)
      {
      continue;
      }
  ...
  this->Renderer->SetDelegate(0);
  rwin->SwapBuffersOn();
}

//----------------------------------------------------------------------------
void vtkKWEVisibleGlyphSelector::SavePixelBuffer(int pass, unsigned char *buff)
{  
  if (pass < 0) 
    {
    pass = 0;
...
//----------------------------------------------------------------------------
void vtkKWEVisibleGlyphSelector::ComputeSelectedIds()
{
  //go over the pixels, determine the proc, actor and glyph id.
  //create a sorted list of hitrecords without duplicates
  vtkstd::set<vtkKWEVisibleGlyphSelectorInternals> hitrecords;
  ...
  vtkKWEVisibleGlyphSelectorInternals nhit, zero;
  ...
  if (hitcnt != 0)
    ...
    vtkIdType glyphid = 0;
    vtkstd::set<vtkKWEVisibleGlyphSelectorInternals>::iterator sit;
    for (sit = hitrecords.begin(); sit != hitrecords.end(); sit++)
      ...
      this->SelectedIds->SetTupleValue(glyphid, info);
      this->PixelCounts->SetValue(glyphid, sit->PixelCount);
      glyphid++;
      }    
    }
}
...
//----------------------------------------------------------------------------
void vtkKWEVisibleGlyphSelector::GetSelectedIds(vtkIdTypeArray *dest)
{
  if (dest==NULL)
    {
    return;
...
//----------------------------------------------------------------------------
void vtkKWEVisibleGlyphSelector::GetSelectedIds(vtkSelection *dest)
{
  if (dest==NULL)
    {
    return;
  ...
  for (vtkIdType i = 0; i < numTup; i++)
    ...
    if (aTuple[1] != lActorId)
      ...
      selection->GetProperties()->Set(
                                      vtkSelection::FIELD_TYPE(), vtkSelection::CELL); // this glyph but we will use cell tag.
      ...
      glyphids = vtkIdTypeArray::New();
      glyphids->SetNumberOfComponents(1);
      selection->SetSelectionList(glyphids);
...
#endif
    glyphids->InsertNextValue(aTuple[3]);
...
//----------------------------------------------------------------------------
//vtkProp* vtkKWEVisibleGlyphSelector::GetActorFromId(vtkIdType id)
//{
...
//----------------------------------------------------------------------------
void vtkKWEVisibleGlyphSelector::PrintSelectedIds(vtkIdTypeArray *lists)
{
  ...
  vtkIdType rec[4];
  for (vtkIdType glyphid = 0; glyphid < lists->GetNumberOfTuples(); glyphid++)
    {
    lists->GetTupleValue(glyphid, rec);    
    cerr << rec[0] << '\t' << rec[1] << "\t\t" << rec[2] << ' ' << rec[3] << endl; 
    }
}

//---------------------------------------------------------------------------
//void vtkKWEVisibleGlyphSelector::SetIdentPainter(vtkIdentColoredPainter *ip)
//{
  //  if (this->Renderer != NULL)
  //    {
...
//
void vtkKWEVisibleGlyphSelector::GetPixelSelection( 
    int displayPos[2],
    vtkIdType & procId,
    vtkIdType & glyphId,
    vtkProp  *& actorPtr )
...
#if (VTK_SIZEOF_ID_TYPE == 8)
      glyphId = (nhit.GetField(2) << 32) | nhit.GetField(3); // cellId
#else 
      glyphId = nhit.GetField(3);                            // cellId
#endif
    }
}

//-----------------------------------------------------------------------------
void vtkKWEVisibleGlyphSelector::PrintSelf(ostream& os, vtkIndent indent)
{
  ...
  os << indent << "DoGlyphIdLo" << this->DoGlyphIdLo << endl;
  os << indent << "DoGlyphIdMid" << this->DoGlyphIdMid << endl;
  os << indent << "DoGlyphIdHi" << this->DoGlyphIdHi << endl;
