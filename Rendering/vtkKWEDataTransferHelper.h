...
//
//=============================================================================
// .NAME vtkKWEDataTransferHelper - is a helper class that aids in transferring
// data between CPU memory and GPU memory.
// .SECTION Description
// vtkKWEDataTransferHelper is a helper class that aids in transferring data
// between the CPU memory and the GPU memory. The data in GPU memory is stored
// as textures which that in CPU memory is stored as vtkDataArray.
// vtkKWEDataTransferHelper provides API to transfer only a sub-extent of CPU
// structured data to/from the GPU.
//
// 

#ifndef __vtkKWEDataTransferHelper_h
#define __vtkKWEDataTransferHelper_h
...
class VTKEdge_RENDERING_EXPORT vtkKWEDataTransferHelper : public vtkObject
{
public:
  static vtkKWEDataTransferHelper* New();
  vtkTypeRevisionMacro(vtkKWEDataTransferHelper, vtkObject);
...
private:
  vtkKWEDataTransferHelper(const vtkKWEDataTransferHelper&); // Not implemented.
  void operator=(const vtkKWEDataTransferHelper&); // Not implemented.
//ETX
};
