...
int vtkKWEImageFFT::IterativeRequestInformation(
...
//----------------------------------------------------------------------------
void vtkKWEImageFFTInternalRequestUpdateExtent(int *inExt, int *outExt, int *wExt,
                                            int iteration)
{
...
int vtkKWEImageFFT::IterativeRequestUpdateExtent(
  vtkInformation* input, vtkInformation* output)
{
  ...
  int inExt[6];
  vtkKWEImageFFTInternalRequestUpdateExtent(inExt,outExt,wExt,this->Iteration);
...
// is always doubles.
template <class T>
void vtkKWEImageFFTExecute(vtkKWEImageFFT *self,
  ...
  for (idx2 = outMin2; idx2 <= outMax2; ++idx2)
    ...
    for (idx1 = outMin1; !self->AbortExecute && idx1 <= outMax1; ++idx1)
      ...
      for (idx0 = inMin0; idx0 <= inMax0; ++idx0)
        {
        pComplex->Real = static_cast<float>(*inPtr0);
        pComplex->Imag = 0.0;
        if (numberOfComponents > 1)
          { // yes we have an imaginary input
          pComplex->Imag = static_cast<float>(inPtr0[1]);;
          }
        inPtr0 += inInc0;
        ++pComplex;
        }
...
// Not threaded yet.
void vtkKWEImageFFT::ThreadedExecute(vtkImageData *inData, vtkImageData *outData,
...
void vtkKWEImageFFT::SetNumberOfThreads(int _arg)
{
