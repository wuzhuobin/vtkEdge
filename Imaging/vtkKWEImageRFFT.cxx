...
vtkCxxRevisionMacro(vtkKWEImageRFFT, "$Revision: 1774 $");
vtkStandardNewMacro(vtkKWEImageRFFT);

// Description:
// Complex number struct using floats as CUDA only supports
// 32-bit floating point percision as of 08/12/2008.
typedef struct{
    float Real;
    float Imag;
  } vtkImageComplexf;

// execution method found in vtkKWEImageFFT.cu
extern "C"
...
int vtkKWEImageRFFT::IterativeRequestInformation(
...
void vtkKWEImageRFFTInternalRequestUpdateExtent(int *inExt, int *outExt,
                                             int *wExt,
                                             int iteration)
{
...
int vtkKWEImageRFFT::IterativeRequestUpdateExtent(
  vtkInformation* input, vtkInformation* output)
{
  ...
  int inExt[6];
  vtkKWEImageRFFTInternalRequestUpdateExtent(inExt,outExt,wExt,this->Iteration);
...
// is always doubles.
template <class T>
void vtkKWEImageRFFTExecute(vtkKWEImageRFFT *self,
                         vtkImageData *inData, int inExt[6], T *inPtr,
                         vtkImageData *outData, int outExt[6], double *outPtr,
                         int id)
{
  vtkImageComplexf *inComplex;
  vtkImageComplexf *outComplex;
  vtkImageComplexf *pComplex;
  //
  int inMin0, inMax0;
  ...
  inComplex = new vtkImageComplexf[inSize0];
  outComplex = new vtkImageComplexf[inSize0];
  ...
  for (idx2 = outMin2; idx2 <= outMax2; ++idx2)
    ...
    for (idx1 = outMin1; !self->AbortExecute && idx1 <= outMax1; ++idx1)
      ...
      ::ExecuteRFft(inComplex, outComplex, inSize0);
...
// Not threaded yet.
void vtkKWEImageRFFT::ThreadedExecute(vtkImageData *inData, vtkImageData *outData,
                                  int outExt[6], int threadId)
{
  ...
  switch (inData->GetScalarType())
    {
    vtkTemplateMacro(
      vtkKWEImageRFFTExecute(this, inData, inExt,
...
int vtkKWEImageRFFT::SplitExtent(int splitExt[6], int startExt[6],
                             int num, int total)
{
  int splitAxis;
  int min, max;
...
void vtkKWEImageRFFT::SetNumberOfThreads(int _arg)
{
